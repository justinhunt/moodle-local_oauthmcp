...
/**
 * RFC 7591 OAuth 2.0 Dynamic Client Registration for this shared MCP authorization server.
 *
...
$ratelimitcache = cache::make('local_oauthmcp', 'dcrratelimit');
...
foreach ($redirecturis as $uri) {
    if (!is_string($uri) || !helper::valid_redirect_uri($uri)) {
...
$now = time();
do {
    $clientid = 'oamc_' . random_string(32);
} while ($DB->record_exists('local_oauthmcp_clients', ['clientid' => $clientid]));
...
$DB->insert_record('local_oauthmcp_clients', $record);
