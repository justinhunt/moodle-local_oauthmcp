...
/**
 * OAuth 2.1 /authorize endpoint for this shared MCP authorization server.
 *
 * Interactive: requires a logged-in Moodle user, shows a consent screen naming the client,
 * and on approval issues a short-lived single-use authorization code (local_oauthmcp_codes).
 * PKCE (S256) is mandatory - there is no non-PKCE code flow. The DCR/manual client is
 * resolved from local_oauthmcp_clients; an https:// client_id (a Client ID Metadata Document
 * reference) is resolved separately via oauth_authorize_resolve_cimd().
 *
 * Unlike a single-resource authorization server, the capability that gates consent depends
 * on which protected resource the client is requesting (see
 * \local_oauthmcp\oauth\registry) - so the capability check happens only once the resource
 * has been resolved, not up front.
 *
 ...
 *         JSON string, matching the local_oauthmcp_clients row shape), or null if the
 *         document could not be fetched or failed validation
 */
function oauth_authorize_resolve_cimd(string $url): ?\stdClass {
    global $CFG;

    $cache = \cache::make('local_oauthmcp', 'cimdclient');
    $cachekey = sha1($url);
...
$PAGE->set_url(new moodle_url('/local/oauthmcp/oauth_authorize.php'));
$PAGE->set_pagelayout('standard');
$PAGE->set_title(get_string('oauthauthorizetitle', 'local_oauthmcp'));
...
// Resolve which registered protected resource this request is for. Real clients sometimes
// omit `resource` entirely; registry::resolve() falls back to the single registered
// resource in that case, and only treats it as ambiguous when more than one is registered.
$resourceobj = registry::resolve($resource);
if ($resourceobj === null) {
    $message = $resource !== null
        ? "resource '{$resource}' is not registered with this authorization server"
        : 'resource is required (more than one protected resource is registered on this server)';
    oauth_authorize_fail('invalid_target', $message, $redirecturiparam, $state);
}

// The capability gate depends on which resource is being requested, so it can only be
// checked now - not up front, unlike a single-resource authorization server.
if (!has_capability($resourceobj->capability, $systemcontext)) {
    echo $OUTPUT->header();
...
if ($confirm === 'approve') {
    ...
    $DB->insert_record('local_oauthmcp_codes', (object) [
        ...
        // Store the canonical registered resource URL, not whatever (possibly absent) value
        // the client sent - so /token can always resolve it deterministically.
    ...
    $params = ['code' => $rawcode, 'iss' => helper::issuer()];
    if ($state !== null) {
        $params['state'] = $state;
    }
...
$consentbody = get_string('oauthconsentbody', 'local_oauthmcp', (object) [
    'clientname' => $client->clientname ?: get_string('oauthunnamedclient', 'local_oauthmcp'),
    'resourcedescription' => $resourceobj->description ?? get_string('oauthconsentbody_defaultresource', 'local_oauthmcp'),
]);
...
echo $renderer->render_oauthconsent([
    ...
    'formurl' => (new moodle_url('/local/oauthmcp/oauth_authorize.php'))->out(false),
    'sesskey' => sesskey(),
