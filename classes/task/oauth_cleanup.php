...
namespace local_oauthmcp\task;

use core\task\scheduled_task;

/**
 * Hourly housekeeping for the OAuth authorization server: expired authorization codes are
 * useless the moment they expire, and revoked refresh tokens are only kept around briefly to
...
class oauth_cleanup extends scheduled_task {
    ...
    public function get_name() {
        return get_string('oauthcleanuptask', 'local_oauthmcp');
    }
    ...
    public function execute() {
        ...
        $now = time();
        $DB->delete_records_select('local_oauthmcp_codes', 'expires < ?', [$now]);
