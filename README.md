...
- **Not a token issuer of its own.** The "access token" it hands back from `/token` is a real Moodle web-service token generated by the consumer plugin (eg mod_minilesson). It most likely 
  has an `external_tokens` row, created the same way the "Manage tokens" admin page does. 
...
| Site-wide AS metadata, PKCE enforcement | `mint_or_reuse_token()` and your service's function set |
...
### a) Let PHP see the `Authorization` header — *required*
...
**Protected-resource forms.** Needed for Gemini Spark. Unlike the AS forms these are
...
<VirtualHost *:443>
    ...
    <IfModule mod_rewrite.c>
        ...
        # Authorization-server metadata -> local_oauthmcp (one AS per site, no collision)
        RewriteRule ^/\.well-known/oauth-authorization-server$ /local/oauthmcp/oauth_metadata.php [L]
        RewriteRule ^/\.well-known/openid-configuration$        /local/oauthmcp/oauth_metadata.php [L]
